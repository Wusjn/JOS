...
sys_cputs(const char *s, size_t len)
{
	...
	//if(!curenv) panic("sys_cputs: no curenv\n");
	//pgdir_walk(curenv->env_pgdir,s,0);
...
sys_exofork(void)
{
	...
	int success;
	struct Env *newenv;
	if((success=env_alloc(&newenv,curenv->env_id))<0) return success;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if(tarpage==NULL) return -E_NO_MEM;
	if((success=page_insert(tarenv->env_pgdir,tarpage,va,perm))<0){
		page_free(tarpage);
		return success;
	}
	return 0;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	int success;
	if((success=envid2env(srcenvid,&senv,1))<0) return success;
	...
	if((perm&PTE_W)&&((*tarpte|PTE_W)==0)) return -E_INVAL;
	if((*tarpte&PTE_P)==0) panic("sys_page_map: tarpte|PTE_P==0\n");
	if((success=page_insert(denv->env_pgdir,tarpage,dstva,perm))<0) return success;
	return 0;
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	struct Env *tarenv;
	pte_t *cpte;
	struct PageInfo *cpage;
	if((success=envid2env(envid,&tarenv,0))<0) return success;
	if(tarenv->env_status!=ENV_NOT_RUNNABLE||tarenv->env_ipc_recving!=1) return -E_IPC_NOT_RECV;
	tarenv->env_ipc_perm=0;
	if((uint32_t)srcva<UTOP){
		if((uint32_t)srcva%PGSIZE!=0) return -E_INVAL;
		if(((perm&(PTE_P|PTE_U))!=(PTE_P|PTE_U))||(perm&~(PTE_P|PTE_U|PTE_W|PTE_AVAIL))) return -E_INVAL;
		if((cpage=page_lookup(curenv->env_pgdir,srcva,&cpte))==NULL) return -E_INVAL;
		if((perm&PTE_W)&&((*cpte)&PTE_W)==0) return -E_INVAL;
		if((uint32_t)(tarenv->env_ipc_dstva)<UTOP&&(success=page_insert(tarenv->env_pgdir,cpage,tarenv->env_ipc_dstva,perm))<0) return success;
		if((uint32_t)(tarenv->env_ipc_dstva)<UTOP) tarenv->env_ipc_perm=perm;
	}
...
sys_packet_try_send(char *addr,uint32_t size){
	user_mem_assert(curenv,addr,size,0);
	return e1000_transmit(addr,size);
}

static int
sys_packet_try_recv(char *addr,uint32_t size){
	user_mem_assert(curenv,addr,size,PTE_W);
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case(SYS_packet_try_recv):
		ret=sys_packet_try_recv((void *)a1,a2);
		break;
	default:
		return -E_INVAL;
	}
