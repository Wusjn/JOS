...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	char *end;
	struct PageInfo *pp;
	begin=(char *)ROUNDDOWN(va,PGSIZE);
	end=(char *)ROUNDUP(((uint32_t)va)+len,PGSIZE);
	for(;begin<end;begin+=PGSIZE){
		pp=page_alloc(0);
		if(!pp) panic("region_alloc : out of memory\n");
		if(page_insert(e->env_pgdir,pp,begin,PTE_U|PTE_W)) panic("region_alloc : page_insert fault\n");
	}
	return;
}
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	for(;ph<eph;ph++){
		...
		if(ph->p_filesz>ph->p_memsz) panic("load_icode: filesz>memsz\n");
...
env_create(uint8_t *binary, enum EnvType type)
{
	...
	cenv->env_type=type;
	cenv->env_parent_id = 0;
	// If this is the file server (type == ENV_TYPE_FS) give it I/O privileges.
	// LAB 5: Your code here.
	if(type==ENV_TYPE_FS){
		cenv->env_tf.tf_eflags |= FL_IOPL_MASK;
	}
}
